...
- Uploaded images, videos, and audio are saved to `cdn/Uploads/`, with thumbnails stored in `cdn/Uploads/thumbnails/` (audio has no thumbnail). Upload metadata is persisted in the Postgres table `media_uploads`, which stores filename, CDN URL, optional thumbnail URL, upload timestamp, file format, creator display name, and uploader label. Files use the naming pattern `<user_id>-<message_id>_<nn><extension>`. The feature reads environment variables `IMAGE_UPVOTE_EMOJI_NAME` and `IMAGE_UPVOTE_THRESHOLD`.
